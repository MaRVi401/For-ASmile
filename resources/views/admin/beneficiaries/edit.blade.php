...
@section('title', 'Edit Profil Penerima')

@section('content')
    <div class="space-y-6 max-w-2xl mx-auto">
        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-2xl font-bold text-slate-800">Edit Profil Penerima Santunan</h2>
                <p class="text-slate-500 text-sm mt-1">Perbarui data identitas nama, nomor kontak, atau alamat rumah
                    penerima.</p>
            </div>
        ...
        <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-xs">
            <form action="{{ route('admin.beneficiaries.update', $beneficiary->id) }}" method="POST" class="space-y-4">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $beneficiary->name) }}"
                ...
                <div>
                    <label for="address" class="block text-sm font-semibold text-slate-700 mb-1.5">Alamat Lengkap
                        Rumah</label>
                    <textarea name="address" id="address" rows="4" placeholder="Masukkan alamat domisili warga..."
                        class="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 bg-slate-50/50 text-slate-800 text-sm focus:border-blue-500 focus:bg-white focus:ring-1 focus:ring-blue-500 outline-hidden transition">{{ old('address', $beneficiary->address) }}</textarea>
                    @error('address')
                        <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
                    @enderror
                </div>
